...
func ValidateEditIntent(intent EditIntent) error {
	...
	case EditIntentKindDelete:
		if intent.Delete == nil {
			return fmt.Errorf("edit intent: delete requires payload")
		}
		return validateTarget(intent.Delete.Target)
	case EditIntentKindInsertImport:
		if intent.InsertImport == nil || strings.TrimSpace(intent.InsertImport.Import) == "" {
			return fmt.Errorf("edit intent: insert_import requires import")
		}
		if !strings.HasPrefix(strings.TrimSpace(intent.InsertImport.Import), "import ") {
			return fmt.Errorf("edit intent: import must start with %q", "import ")
		}
		return nil
	case EditIntentKindCreateFile:
		if intent.CreateFile == nil || strings.TrimSpace(intent.CreateFile.Path) == "" {
			return fmt.Errorf("edit intent: create_file requires path")
		}
		return nil
	case EditIntentKindAppendToFile:
		if intent.AppendToFile == nil || strings.TrimSpace(intent.AppendToFile.Path) == "" || strings.TrimSpace(intent.AppendToFile.Text) == "" {
			return fmt.Errorf("edit intent: append_to_file requires non-empty path and text")
		}
		return nil
	default:
