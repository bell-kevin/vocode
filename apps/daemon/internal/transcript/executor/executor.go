...
// Executor runs one voice.transcript through the agent: an iterative loop of NextIntent
// (each pass uses [agentcontext.TurnContext]), optional request_context rounds, retries,
...
func (e *Executor) Execute(params protocol.VoiceTranscriptParams, gatheredIn agentcontext.Gathered, extSucceeded []intents.Intent, extFailed []agentcontext.FailedIntent) (protocol.VoiceTranscriptResult, agentcontext.Gathered, *agentcontext.DirectiveApplyBatch, bool) {
	...
	st := &agentLoopState{
		...
		directives: make([]protocol.VoiceTranscriptDirective, 0, maxLoopIters),
		maxRetries: maxRetries,
	}

	for range maxLoopIters {
		adv, failRes, abort := e.runOneAgentLoopIteration(params, text, hostCursor, extSucceeded, extFailed, st)
		if abort {
	...
	return finalizeExecute(st, maxLoopIters)
}
